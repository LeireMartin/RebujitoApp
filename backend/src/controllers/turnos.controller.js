...
export async function iniciarTurno(req, res, next) {
  try {
    ...
    if (turnoActivo) return conflict(res, 'Ya hay un turno activo en esta partida')
    ...
    const fase   = partida.faseActual === 0 ? 2 : partida.faseActual
    ...
    const turno = await prisma.turno.create({
      data: { partidaId, equipoId, fase, numero, estado: 'ACTIVO', inicio: new Date() },
    })
...
export async function registrarResultado(req, res, next) {
  try {
    ...
    if (turno.estado !== 'ACTIVO') return conflict(res, 'El turno no está activo')
    ...
    ok(res, { siguientePalabra, palabrasRestantes, adivinadasTurno, faseCompletada: !siguientePalabra })
  } catch (err) { next(err) }
}
...
export async function finalizarTurno(req, res, next) {
  try {
    ...
    if (!turno)                    return notFound(res, 'Turno no encontrado')
    if (turno.estado !== 'ACTIVO') return conflict(res, 'El turno ya ha finalizado')

    const adivinadas = turno.resultados.filter(r => r.resultado === 'ADIVINADA').length
    const pasadas    = turno.resultados.filter(r => r.resultado === 'PASADA').length
    ...
    const totalPalabras  = await prisma.palabra.count({ where: { partidaId: turno.partidaId } })
    const faseCompletada = usadas.length >= totalPalabras
    ...
    if (faseCompletada) {
      ...
      if (nuevaFase > 4) {
        await prisma.partida.update({ where: { id: turno.partidaId }, data: { estado: 'FINALIZADA' } })
        partidaFinalizada = true
      } else {
        await prisma.partida.update({ where: { id: turno.partidaId }, data: { faseActual: nuevaFase } })
    ...
    ok(res, { adivinadas, pasadas, puntosGanados: adivinadas, siguienteEquipo: siguienteEquipo.nombre, faseCompletada, siguienteFase, partidaFinalizada })
  } catch (err) { next(err) }
}